...
2. 검색 기능 구현
   ...
   - Input 클릭시 border color가 드러나게 효과 적용.
...
6. 하단에 위로 가기 버튼 추가
   ...
   - 위로 가기 클릭시 window scroll이 움직이고 AniList의 scroll은 움직이지 않는 문제가 발생 -> 기존의 window 기능을 제거하고 AniList의 scroll이 작동하게 변경.
...
10. 시스템 테마에 따라 자동으로 배경색이 변경. -> 전체에 적용해 보니 리스트 타입에 자료에는 height의 상한선을 설정하기 어려운 문제가 발생했다. -> Home에만 국한되도록 수정함.

11. Home에 전체 자료가 출력되게 변경. -> 색상은 새로고침할 때미다 랜덤색상 적용. -> 페이지와 어울린다기 보다 너무 튀어보여 흑백으로만 보이게 수정.
